...
export default function ReportsPage() {
  ...
  const [candidates] = useState([
    { 
      ...
      aiStats: {
        ...
        promptScore: "12.5%",
        history: [
          { time: "14:12", score: "92.14%", prompt: "8.2%" },
          { time: "14:18", score: "94.50%", prompt: "9.5%" },
          { time: "14:26", score: "97.80%", prompt: "11.4%" },
          { time: "14:32", score: "99.12%", prompt: "12.5%" }
        ]
      }
    },
    { 
      ...
      aiStats: {
        ...
        promptScore: "4.2%",
        history: [
          { time: "10:05", score: "12.45%", prompt: "2.1%" },
          { time: "10:15", score: "15.22%", prompt: "4.2%" }
        ]
      }
    },
    { 
      ...
      aiStats: { checkCount: 0, promptScore: "0%", history: [] }
    }
  ]);
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 flex flex-col h-screen overflow-hidden">
      {isAdmin ? (
        <>
          <header className="flex-shrink-0">
             ...
             <p className="text-[10px] font-black text-[#94A3B8] uppercase tracking-[0.3em] mt-1">Authorized Oversight & Scrutiny Panel</p>
          </header>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0 overflow-hidden pb-6">
             <div className="lg:col-span-3 bg-white rounded-[24px] border border-[#E2E8F0] shadow-sm flex flex-col overflow-hidden min-h-0">
                <div className="p-8 border-b border-[#F1F5F9] space-y-6 flex-shrink-0">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                   {candidates
                     ...
                     .filter(c => c.report || c.aiStats.checkCount > 0)
                     .map(c => (
                     <button 
                       key={c.id}
                     ...
                     >
                        <div className="flex items-center justify-between mb-1">
                           ...
                           <span className={`text-[8px] font-black uppercase tracking-[0.2em] ${selectedCandidate?.id === c.id ? "text-white/40" : "text-[#94A3B8]"}`}>Ready</span>
                        </div>
                        <h4 className="text-xl font-extrabold tracking-tight">{c.name}</h4>
                        <p className={`text-[10px] font-bold mt-2 ${selectedCandidate?.id === c.id ? "text-white/60" : "text-[#94A3B8]"}`}>
             ...
             <div className="lg:col-span-9 flex flex-col gap-10 overflow-y-auto custom-scrollbar pr-2 min-h-0">
